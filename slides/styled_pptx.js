// Styled weekly decks for Culture as Data. Reads slides.json, writes slides/pptx/.
const pptxgen = require("pptxgenjs");
const fs = require("fs");
const path = require("path");
const weeks = JSON.parse(fs.readFileSync(path.join(__dirname,"slides.json"),"utf8"));
const OUT = path.join(__dirname,"pptx");
...
function build(w){
  ...
  if(w.n===1){
    try{
      ...
      if(files.length){
        ...
        s.addText("Real CC0 paintings from the Met's open-access collection, downloaded live in the notebook",{x:M,y:1.1,w:11.9,h:0.7,fontFace:SERIF,fontSize:26,bold:true,color:INK});
        files.forEach((f,i)=>{
        ...
        s.addText("Counted by brightness in the session; by defined color ranges after class. The notebook downloads its own set live.",{x:M,y:7.0,w:11.9,h:0.35,fontFace:SANS,fontSize:12,italic:true,color:MUTED});
      }
  ...
  const fsz=rows.length>7?11:12.5;
  rows.forEach((r,i)=>{
    const y=top+i*rh;
    ...
    s.addText(r.a,{x:M+1.15,y:y,w:10.55,h:rh,fontFace:SANS,fontSize:fsz,color:INK,valign:"middle",autoFit:true});
  });
  ...
  // 7. How this week connects (through-lines)
  s=pres.addSlide(); s.background={color:WHITE};
  s.addText("HOW THIS WEEK CONNECTS",{x:M,y:0.7,w:11,h:0.35,fontFace:SANS,fontSize:13,bold:true,color:TERRA,charSpacing:3});
  s.addText("The course's through-lines",{x:M,y:1.08,w:11,h:0.7,fontFace:SERIF,fontSize:28,bold:true,color:INK});
  (w.threads||[]).slice(0,3).forEach((t,i)=>{
  ...
  return pres.writeFile({fileName:OUT+"/week-"+String(w.n).padStart(2,"0")+".pptx"}).then(f=>console.log("wrote",f));
}
