...
| `transformers_playground.ipynb` | any | grab bag, run what you need | GPU check + Hugging Face + tokens + ModernBERT in context, layer by layer + CLS vs. mean vs. trained pooling + semantic search over two novels + CLIP search, zero-shot labels, patches and attention + a CLIP trained from scratch |
