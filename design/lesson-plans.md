...
**Featured study — the word2vec analogy (Mikolov et al. 2013).** *king − man + woman* lands near *queen*: a model told nothing about royalty or gender, trained only to guess neighbouring words, ends up carrying those relationships as directions in space. **Interrogate (room first):** it is also a magic trick. The arithmetic excludes the query words, or *king* returns as its own nearest neighbour; most analogies fail and the famous four are the ones that worked; and the same geometry produced *doctor − man + woman = nurse* (Bolukbasi 2016). What the vectors know, they learned from us — which is Week 5's argument in advance.
...
**Reading:** Alammar, "The Illustrated Word2Vec" — where word vectors come from, drawn one picture at a time. **The lecture assumes the pictures, so it has to be read beforehand** (~20 min).
**Supplement:** Krause, "Data Biographies" (We All Count), the frame for this week's sketch; Freelon, "Computational Research in the Post-API Age," on what happens when the front door closes.
