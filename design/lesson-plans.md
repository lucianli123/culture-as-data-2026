...
**Data:** NYT Wedding Announcements (7,835 rows, The Upshot's public CSV, snapshotted in `notebooks/data/week01/`); Shakespeare's 154 sonnets (Gutenberg #1041, snapshotted); 18 Met Museum thumbnails with manifest (CC0, snapshotted).
...
| 0:35 | **The vocabulary and the map** (no code): corpus, method, model, embedding — plain language, pictures — plus **what data itself is**. Anatomize a CSV on screen: one row per observation, one column per recorded fact, a header naming them; ours has four columns and 7,835 rows. Then Drucker's correction: data ("the given") is the wrong word — *capta*, "the taken," is honest, because someone chose what to record. The wedding table proves it: a name status for the bride and none for the groom, ages but no occupations. **Two minutes in pairs:** what did this table's makers take, and what did they leave? Deliverable shown (notebook + web page). |
| 0:47 | **Lab 1 (worked, participatory, 33 min):** copy the notebook to Drive and **mount Drive into the runtime** (this *is* setup — and it's how your corpus and results survive Colab wiping the session; everything saves to one project folder in your Drive), Gemini key into Colab Secrets, load wedding data from URL. Instructor narrates, plants one real bug, and recovers it *out loud using the unblocking kit* — read the last line of the error, paste it back to the AI with "this errored, fix it and tell me what went wrong," try twice, then ask a human. **The notebook's guided section on working with the assistant (ask precisely, predict, run, interrogate) is the take-home for this beat; the deep-research rules (scout, then close the tool) are on the deck. Hand out the one-page "common errors" cheat sheet and the "how to read a code cell" page here** (`kits/reading-a-code-cell.md` — the five patterns that cover nearly every cell in the course; the reading vocabulary behind competency 1); the AI *will* hand them a traceback, and the move is to not panic. First chart. Then solo: with a partner, draft three questions this data could answer, pick one, have the assistant write the code, run it, chart it. **The Week 1 check lives here:** before running your solo cell, write one sentence saying what it will do. (Trace it; C1.) |
| 1:20 | Break |
| 1:30 | **Lab 2 (worked, 22 min) — count words, then pixels.** Shakespeare's 154 sonnets (snapshotted in the repo): *thy* and *thou* top the count until the stop list learns Early Modern English — then *love* (195 uses), *beauty*, and *time* surface, the corpus's reputation recovered by counting, and the stop list exposed as a per-corpus choice. Then the Met image corpus, ranked darkest to brightest. The notebook's remaining sections — defined color ranges, and **Sonnet 130's bag against the poem itself** (a catalogue of praise the poem negates line by line, the largest possible bag-of-words loss) — are the **after-class half** (~20 minutes, named in the Sketch), the guided bridge to Week 2. |
