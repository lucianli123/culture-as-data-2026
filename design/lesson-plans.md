...
| 1:20 | Break |
| 1:30 | **Lab 2 (worked, 22 min) — count pixels.** The manifest is the room's first CSV (one row per painting, one column per fact the Met recorded), and the 18 snapshotted paintings rank darkest to brightest by average luminance — a picture is numbers, counted with the same technique as the words. The notebook's remaining sections — defined color ranges, and **Sonnet 130's bag against the poem itself** (a catalogue of praise the poem negates line by line, the largest possible bag-of-words loss) — are the **after-class half** (~20 minutes, named in the Sketch), the guided bridge to Week 2. |
