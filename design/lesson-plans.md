...
**Sketch:** one question from your life answerable with text or image data; three sentences. *Optional, for anyone who wants the pieces in hand before Week 2:* the `python_warmup` notebook, twenty minutes of predict-then-run — never required, never quizzed.
...
| 0:38 | **[Workshop] Hand-built bag-of-words (22 min):** two authors, highlighters, tally the top words. Argue about merging run/running, casing, stop-words. The arguments *are* the lesson: counting requires defining. Close the block with the loss demonstration (in the notebook): two sentences with identical bags and opposite meanings ("the critics loved it, the public did not"), then a picture beside its shuffled pixels — same average color, same brightness, same histogram, and only one is a picture. Order and composition are what the bag trades away for scale; Week 5's embeddings are the answer to exactly this loss. |
| 1:00 | Break |
