...
**Data:** Shakespeare's 154 sonnets (Gutenberg #1041, snapshotted in `notebooks/data/week01/`); 18 Met Museum thumbnails with manifest (CC0, snapshotted). Everything runs with no network.

| Time | Activity |
|---|---|
| 0:00 | **Setting the room.** Logistics in two minutes (the rhythm: reading → session → sketch → check; accounts needed; everything on the site; $0). Social rules demonstrated — instructor deliberately breaks one and is corrected. **Icebreaker, one fast round:** name, one piece of culture you kept returning to this year, one number about it you wish you knew — keep your answer, it is a first draft of a project question. Announce today's live coding includes real mistakes on purpose. |
| 0:10 | **[Lecture] The stakes — why read culture with machines (18 min):** machines already read culture at scale — the feed ranking what you see, the moderation filter, the model trained on scraped art and prose — and that reading happens without you. Then the evidence that doing it yourself yields real knowledge: the screenplay verb split (women *snuggle, giggle, sob*; men *strap, gallop, kill* — and 85% of the writers were men), pop music's 1991 hip-hop revolution outranking 1964, the Rowling stylometry unmasking, the man-is-to-programmer analogy, the $432,500 AI portrait trained on other people's paintings. Each is a count; each has arguable choices; both halves are this course. The close: in ten weeks you publish a checkable finding of your own. |
| 0:28 | Look at This, then Question It: *She Giggles, He Gallops*. |
| 0:35 | **The vocabulary and the map** (no code): corpus, method, model, embedding — plain language, pictures — plus **what data itself is**. Anatomize a CSV on screen using the painting manifest: one row per painting, one column per fact the Met chose to record. Then Drucker's correction: data ("the given") is the wrong word — *capta*, "the taken," is honest, because someone chose what to record. The manifest proves it: the artist's name but not the sitter's, a free-text date — and what is absent is a decision too. **Two minutes in pairs:** what did the catalogers take, and what did they leave? Deliverable shown (notebook + web page). |
| 0:47 | **Lab 1 (worked, participatory, 33 min):** copy the notebook to Drive and **mount Drive into the runtime** (this *is* setup — it's how work survives Colab wiping the session), Gemini key into Colab Secrets, then **load the 154 sonnets and make the first chart** — *love* across the sequence, a complete investigation in three cells. Count the whole corpus and watch the stop list matter (*thy* and *thou* until the list learns the corpus; then *love*, 195 times). Instructor narrates, plants one real bug, and recovers it *out loud with the unblocking kit* — read the last line, paste it back to the AI, try twice, then ask a human. **Hand out the "common errors" cheat sheet and the "how to read a code cell" page here.** Then the AI loop, solo: with a partner, draft three questions the sonnets could answer, pick one, have the assistant write the cell, predict it, run it. **The Week 1 check lives here** (trace it; C1). The guided assistant section is the take-home; the deep-research rules are on the deck. |
| 1:20 | Break |
| 1:30 | **Lab 2 (worked, 22 min) — count pixels.** The manifest is the room's first CSV (one row per painting, one column per fact the Met recorded), and the 18 snapshotted paintings rank darkest to brightest by average luminance — a picture is numbers, counted with the same technique as the words. In session too: the corpus palette as a bar chart of defined color ranges, and the scramble demonstrations (a sonnet's words shuffled, a painting's pixels shuffled — identical bags, only one a poem or a picture). The **after-class half** (~15 minutes, named in the Sketch): the boundary experiment (widen "reds" and re-count) and **Sonnet 130's bag against the poem itself**, the guided bridge to Week 2. |
| 1:52 | One-slide teaser of the ladder ahead; the standing rituals named. Check-out. |

**Pacing.** This is the most front-loaded session of the term (room-setting, the stakes lecture, vocabulary, two labs, the error drill). The lecture holds its 18 minutes — the stakes are why people stay — and Lab 2 is deliberately light in session (words and brightness only) because the color-range and bag-loss sections are the notebook's after-class half by design. If time still runs short, Lab 2's word count folds into Lab 1's solo turn; the pixel half is protected, as images' only Week 1 beat.
...
**Sketch:** browse pudding.cool and pick a favorite piece — one sentence on what it counted — then write the question from your own life it inspires (three sentences). Plus the notebook's after-class half (define color ranges; what every bag throws away, ~20 minutes), the guided bridge to Week 2. *Optional:* the `python_warmup` notebook for anyone who wants extra reps — never required, never quizzed.
...
| Time | Activity |
|---|---|
| 0:00 | **Warm-up: favorite Pudding pieces (5 min).** From the sketch: what did yours count, and one choice its makers made? Two or three aloud; the habit of admiring and questioning, now run by the room. Then retrieval. |
...
**Reading:** Matt Daniels, "The Largest Vocabulary in Hip Hop" (The Pudding, ~10 min) — 150 rappers ranked by unique words in their first 35,000 lyrics. Today's session in the wild: token analysis, a denominator chosen to make artists comparable, honest caveats (pimp/pimpin count separately; genre drives the number). The assignment: name three choices Daniels made.
**Supplement (for the curious):** the field's own version of today's trial — Underwood, "The Historical Significance of Textual Distances" (2018, arXiv) vs. Da, "The Computational Case Against Computational Literary Studies" (*Critical Inquiry* 2019), with the free CI online forum where the field answers and Da answers back. Da's objections are this session's traps by name; Underwood's answer (anchor distance socially) is where Week 3 begins.
**Deeper (optional):** Grimmer & Stewart, "Text as Data" (Political Analysis 2013), §1–4 — all quantitative models of language are wrong; some are useful; validate everything.
...
**Promise:** argue out the genre article that made this method famous, watch a model get built in practice (and mis-built), then build your own in a group — a logistic regression you can read, doubt, and defend — and leave knowing what Week 4 needs from you.
**Mode balance:** discussion ~35 (the Underwood article, argued by the room) + ~13 report-backs, lecture ~25 (modelling in practice, built live and mis-built), workshop ~32 (group modelling in the blank Colab notebook) — three near-equal thirds, with the article discussion opening the session and the group work closing it.
...
**Data:** each group picks its own labeled pair in the blank notebook — two subreddits pulled live (analyze-only; the default is **r/sandiego vs. r/bayarea**, two regions, which lands near 0.75 against a 0.50 baseline with weights readable at a glance; r/sandiego vs. r/SanDiegan, the *same* city twice, is offered as hard mode once the easy case works) or, with no network, sentences from two novels. Any provided **pre-labeled** set works identically (~1,000 rows — e.g. r/AmItheAsshole posts tagged YTA/NTA, or movie reviews tagged pos/neg). Teachable Machine, now a sixty-second lecture beat, still uses students' own faces/objects via webcam.

| Time | Activity |
|---|---|
| 0:00 | **Warm-up + retrieval (5 min):** last week's shuffle test in one sentence, and one thing you still don't trust about it. |
| 0:05 | **[Discussion] Underwood's "The Life Cycles of Genres" (35 min) — the session opens on the reading.** The room argues the article, not the instructor: what exactly did Underwood count; who decided which novels were detective fiction, and on what evidence; what do the figures claim, and what would have to be true for you to believe them? The best passage in the paper is an error — the model misreads Pynchon's *The Crying of Lot 49*, a detective-fiction spoof — so genre boundaries turn out to be real but fuzzy, and the mistake is what shows it. Close on the transfer question every group carries into the second hour: **where would YOUR classifier fail, and what would that failure teach?** |
| 0:40 | **[Lecture] Modelling in practice (25 min):** a model is four things — features, weights, a fit, and a score you would believe — and the instructor builds one live, mistakes included: turn text into a matrix, split off rows the model never sees, fit it, compare against a baseline that always guesses the bigger pile, then **read the signed weights**. Then what actually goes wrong: scoring on the training rows (that measures memory, not reading), an imbalanced pair flattering itself, a leaked giveaway feature, and the model with no *neither* box — hand it Shakespeare and it answers confidently anyway. Two browser tools close the lecture. **Quick, Draw!** first, for sixty seconds: everyone doodles, a classifier guesses, and the room has met a working classifier before seeing a line of code (its 50-million-drawing training set is also the cleanest example of where labels come from). Then **Teachable Machine**: a cat/dog model trained on nothing but orange cats and brown dogs, the room predicting what the black cat does. Bias is not a ghost in the machine; it is the training set, and you assembled it. **For image projects:** that *is* a classifier on pixels — the same move, different features. |
| 1:05 | Break |
| 1:15 | **[Workshop] Group modelling in Colab (32 min):** threes, one screen, rotating **driver / reader / skeptic**. `week03_modeling_TOGETHER.ipynb` is guided but blank — each station explains the concept, names the tools, and says how to tell whether the result is right, then leaves the code, and most of the how, to the group and its AI across thirty-five empty cells. Stations 7 to 9 ask them to design the experiment first — what they expect to move the score, and what result would change their mind — before touching a dial. It closes on an open bench: three cells with no instructions at all. The setup and the provided tools are Colab form cells, collapsed to a title bar, so the notebook reads as stations rather than as a wall of code. Written for them: a worked six-sentence warm-up that shows the whole pipeline in miniature, and the corpus loader. Pick two piles; build the features and say what the vectorizer's defaults silently decided; fit; judge the score against `DummyClassifier` and look at the confusion matrix; read the top weights and sort them into **topic / register / community habit**; break it with an input from neither pile; then move to the **workbench**, where the plumbing is provided because by now they have built the pipeline once by hand: `fit_model()` sweeps one decision at a time (`min_df`, tf-idf, bigrams, `C`, `class_weight`, stop words) into a single comparison table, with sliders for groups who think faster that way. One model is a result; several are an argument. The two questions the table has to answer: does the accuracy move, and do the top words change more than the accuracy does? Then a choose-one: **is that word a finding or a giveaway?** (the top words on the default pair are place names — *diego*, *sd*, *bay*, *oakland* — so `why("word")` shows the word in use and its evidence, and then the group removes the giveaways with `stop_words=[...]` and refits: if the accuracy collapses the difference *was* the names, and if it holds, the words underneath are the finding), or **change the corpus, not the model** (`use_pair("coffee", "espresso")`, refit the same configuration, and ask which findings survive a different pair). |
| 1:47 | **[Discussion] Report-backs + what Week 4 needs (13 min):** ninety seconds a group — the pair you chose and why, baseline against best, your two word lists sorted into topic/register/habit, where it broke, and the one caveat you'd put in writing. Ten groups' answers are the session's actual finding. Then, compressed into one slide each because the group work now owns the hour: the **methods menu** so Week 4's choice is informed (counting, classification, embeddings in Week 5, annotation in Week 7, plus optional add-ons — character networks, sentiment arcs with Syuzhet's smoothing controversy attached, CLIP image search, and fine-tuning ModernBERT for the technically comfortable), and the **corpus-existence rule**: no proof, no pitch. |
| 1:58 | **Gemini-free check + check-out.** |

**Reading:** Underwood, “The Life Cycles of Genres” (*Cultural Analytics* 2016) — **the article the session discusses, so it has to be read before class.** Read the argument and the figures; skim the model tables. Bring the three questions: what did he decide, what is the load-bearing assumption, where is the gap? (~30 min.)
**Supplement:** browse *America's Public Bible* online (americaspublicbible.org) — the same tool as finished scholarship you can poke at, and the model for your own deliverable.
**Deeper (optional):** Underwood, *Distant Horizons* (2019), chapter 2 — the book-length version of today's article, with the Pynchon misread in context.
...
**Focus:** the lecture week. Two ideas — a network is stacked classifiers, and a word vector is what falls out of a small network trained on a fake task — plus the practical half hour that makes "bring your own corpus" real.
**Competencies:** 2, 6.
**Promise:** see what your Week 3 classifier becomes when you stack it, learn where word vectors come from and what they can and cannot do, and leave with your corpus collected, saved and committed to.
**Mode balance:** lecture ~55 (neural networks, then embeddings), demo + workshop ~40 (APIs, scraping, collect-and-build), discussion ~15 (the analogy's caveats, and pitching in pairs). This is the lecture-heaviest week of the course, deliberately: Week 5 spends its whole session hands-on with these ideas.

**Featured study — Garg, Schiebinger, Jurafsky & Zou, "Word embeddings quantify 100 years of gender and ethnic stereotypes" (PNAS 2018).** Train word vectors on each decade of American text from 1910 onward, measure how far occupation words sit from woman-words versus man-words, and read a century of stereotype change off the geometry — validated against census and survey data. This is the week's argument in one paper: the embedding stops being a gadget and becomes a measuring instrument. **Interrogate (room first):** the measurement tracks the *corpus*, not the country — Google Books and COHA are what got printed and digitised, so a shift may be a shift in publishing. The word lists are the authors' choices. And the bias it detects is the bias in the text, which is the same geometry that makes *king − man + woman = queen* work; you cannot keep one and refuse the other.

**Why this sits here.** Week 3 built a classifier whose weights you could read. Week 5 spends two hours embedding real corpora. Between them the room needs the mechanism: what a network is, what training means, and where a vector comes from. Teaching it in Week 4 buys Week 5 a session that is entirely hands-on.

| Time | Activity |
|---|---|
| 0:00 | **Warm-up (5 min).** Where Week 3 left off, one sentence on where today goes: the classifier you built is one neuron, and today it grows. |
| 0:08 | **[Discussion] Look at This, then Question It (7 min).** The Garg figure: a century of stereotype change, measured as distance in an embedding space. Then the two questions the room should raise before the instructor does — is that the country changing or what got printed, and who chose which words count as female? |
| 0:15 | **[Lecture] From one neuron to a network (20 min).** Your logistic regression, redrawn: inputs, one weight each, a sum, a squash — that is a neuron, and you already built one. Then stack: a hidden layer between the words and the answer, and what stacking buys you, which is features nobody hand-wrote. Training as rolling downhill in fog: a loss is one number saying how wrong you are, gradient descent nudges every weight in the direction that lowers it, repeat. No calculus. **TensorFlow Playground live on the spiral**, watching hidden units carve the space, is the demo that does the work here. Close on the cost: the weights stop being readable, which is exactly the trade Week 7 asks them to accept knowingly. |
| 0:30 | **[Lecture] Word embeddings (30 min).** The problem first, because they have been living with it: to a bag of words, *happy* and *joyful* are unrelated columns, and so are *Oakland* and *Berkeley*. Then the distributional idea — a word is known by the company it keeps — and the training rule in one sentence: **same context, pull together; different context, push apart.** For a word, a context is a few words of text. That is contrastive learning, and word2vec's skip-gram is exactly it. Then the geometry: nearest neighbours, cosine (the same cosine as Week 2's vector space), and the analogy with its caveats *attached rather than added later*. **Embedding Projector live** on a real vocabulary, with a word from a student's corpus if one is to hand. The demo notebook (`week04_demo.ipynb`) has the whole mechanism in runnable code — the pair list, the random negatives, ninety seconds of visible pulling and pushing, then neighbours and the analogy trick failing honestly on a small corpus — so this can be shown live rather than described, and re-run afterwards by anyone who wants it. Close on pixels, five minutes: **a convolution written out in full** — nine numbers laid over nine pixels, multiply, add, one number out, slide one across — then a filter bank and a three-layer stack on a Met painting, where edges become corners and corners become regions. **CLIP** is the same rule with the context swapped: a painting and *its own* caption pulled together, anyone else's pushed away, two kinds of thing forced into one space. The notebook trains a small one on eighteen Met paintings and then searches them by typing, which is the moment the idea lands. Then one line each on static versus contextual vectors, and on the sentence embeddings Week 5 actually uses. |
| 1:00 | Break |
| 1:10 | **[Demo] Where a corpus comes from — APIs and scraping (20 min),** live, with the AI writing the code. **Route 1, the prepared file**, which is what most projects should use: `pd.read_csv(url)`, `gdown`, `load_dataset()`. **Route 2, an API:** call the Met or Art Institute endpoint (no key), and name *endpoint, key, pagination, rate limit* as each appears on screen; JSON becomes a table in front of them. **Route 3, a small scrape** with BeautifulSoup, only when there is no file and no API, with the four-line check attached: read `robots.txt` and the terms, request slowly, take only what you need, never republish. The licensing one-pager compresses to three sentences here: CC0 and public domain go anywhere; academic sets and community text are analyze-don't-redistribute; shadow libraries never. |
| 1:30 | **[Workshop] Collect-and-build (20 min):** point the cookbook notebook at *your* corpus, reshape it with the AI, and **save it to your Drive project folder** so it is there next week. **Fork the publishing template with GitHub Pages enabled**, so a live placeholder URL exists from today rather than under deadline in Week 9. |
| 1:50 | **[Discussion] Pitch in pairs (two minutes each), then commit.** Corpus, two methods, what would count as a finding. The **pivot kit** is named as insurance, and a null result named as a real result: "I expected X, the data doesn't show it, here's how I know" is a complete project. Check-out. |

**What this costs, and the mitigation.** The old Week 4 gave forty minutes to whole-room pitches, and that is where the instructor picked up tractability problems. Pitching in pairs does not surface those to the front of the room, so the instructor should rove during the collect-and-build lab with the same ear, and the Week 5–6 one-on-ones become the real safety net rather than a nice-to-have. The corpus-existence proof (set as Week 3 homework) does more of the work now: no proof, no pitch, still holds.

**Reading:** Garg, Schiebinger, Jurafsky & Zou, "Word embeddings quantify 100 years of gender and ethnic stereotypes" (PNAS 2018) — abstract, significance statement and figures; methods skimmed. **This is what the lecture's machinery is for**, so it has to be read beforehand (~30 min).
**Supplement:** Alammar, "The Illustrated Word2Vec," where the vectors in that paper come from, drawn one picture at a time; Krause, "Data Biographies" (We All Count), the frame for this week's sketch; Freelon, "Computational Research in the Post-API Age," on what happens when the front door closes.
**Deeper (optional):** 3Blue1Brown's neural-network chapters 1–2, the same lecture with better animation; then Mikolov et al. (2013), the four-page paper that started it.
**Sketch:** write the **Data Biography** (~400 words) for your corpus and *actually collect it* with the cookbook notebook, so you arrive at Week 5 with real data. Then open the **Embedding Projector**, search a word from your own corpus, and note one neighbour that makes sense and one that does not.
**Check (explain it):** what a word vector is, in your own words, and one thing it cannot tell you — plus your question aloud, what it omits, and where your data comes from. *(C2, C6.)*
...
| 0:07 | **Kill it, then keep what survives (45 min, one continuous arc):** the robustness habits, run live on *your own* results — shuffle the labels and re-run (does the pattern survive randomization?); split the corpus in half (does it hold in both?); ask "compared to what?" (every claim needs a baseline); and for AI-labeled projects, paraphrase the codebook and re-run the annotator — labels that swing with the wording are prompt-fragile, and the essay must say so. This is the Bollen rebuttal move from Week 2, done to your own work. Then, in the same sitting, compress whatever survived into one headline sentence, with the room's help. Killing and settling are one motion: what's left standing *is* the finding. *If projects need the time to settle, reduce this to a single robustness check — the one that most threatens the finding — plus the compression step.* |
| 0:52 | Break |
| 1:02 | **Gemini-free writing block (20 min):** write your one-sentence interpretation yourself, AI closed. Yours goes in the essay. |
| 1:22 | **Writing workshop (28 min):** opening question → what you found → what it does *not* show → the choices you made. Distill the Data Biography into the corpus note. Close on your main chart while the draft is open — **interrogate it** (does the y-axis start at zero? is the eye pulled toward a pattern the data won't support? what does it hide?) and fix one thing: the making-side mirror of Week 6's hand-labeling. |
| 1:50 | **Check + check-out.** |

**Pacing.** The robustness arc reduces to a single check when projects need settling (stated in the flow), and the discussion third gives before the writing does. The deeper reading (Da) is for settled projects; do not hand it to a wobbling one this week.
...
| 0:50 | Break |
| 1:00 | **Build lab (50 min):** the template is already live on Pages from Week 4; open the template you forked in Week 4; paste outline, charts, code; render; deploy to Pages. Static by default; JS opt-ins hit the **code-review checkpoint** — read the AI's JavaScript before you ship it. |
| 1:50 | Phone test in pairs; check-out. |

**Pacing.** The build block now runs fifty minutes against a page that has been live since Week 4, and the chart interrogation moved into Week 8's writing workshop — the two changes that make this session survivable. If deployment problems still absorb the room, the structured critique compresses from 35 to 20 minutes and continues asynchronously; a deployed URL is the point of the session.
...
|---|---|---|
| 1 | Wedding CSV · Met thumbnails (the day-one image corpus for pixel counting) | All snapshotted in `notebooks/data/week01/` with a manifest and derived catch-up files — the session runs with no network at all. |
