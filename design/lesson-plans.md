...
| 0:30 | **[Lecture] Word embeddings (30 min).** The problem first, because they have been living with it: to a bag of words, *happy* and *joyful* are unrelated columns, and so are *Oakland* and *Berkeley*. Then the distributional idea — a word is known by the company it keeps — and **word2vec as a small neural net trained on a fake task**: given a word, guess its neighbours. Nobody wants the predictions; the by-product is the point, one vector per word from the hidden layer. Then the geometry: nearest neighbours, cosine (the same cosine as Week 2's vector space), and the analogy with its caveats *attached rather than added later*. **Embedding Projector live** on a real vocabulary, with a word from a student's corpus if one is to hand. Close with one line each on static versus contextual vectors, and on the sentence embeddings Week 5 actually uses. |
| 1:00 | Break |
