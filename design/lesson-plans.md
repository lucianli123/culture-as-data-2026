...
| 1:20 | Break |
| 1:30 | **Lab 2 (worked, 22 min) — count words, then pixels.** The real headlines' top words (stop-words dominate until removed — the first counting decision), then the Met image corpus ranked darkest to brightest by average brightness. Text and images, counted with the same technique; image projects begin here. The notebook's remaining sections — defined color ranges and what every bag throws away — are the **after-class half** (~20 minutes, named in the Sketch), the guided bridge to Week 2. |
