...
| 0:30 | **[Lecture] Word embeddings (30 min).** The problem first, because they have been living with it: to a bag of words, *happy* and *joyful* are unrelated columns, and so are *Oakland* and *Berkeley*. Then the distributional idea — a word is known by the company it keeps — and the training rule in one sentence: **pull the pairs that really occurred together closer, push randomly drawn pairs apart, repeat.** That is contrastive learning, and word2vec's skip-gram is exactly it; so is CLIP, on pictures and captions. Then the geometry: nearest neighbours, cosine (the same cosine as Week 2's vector space), and the analogy with its caveats *attached rather than added later*. **Embedding Projector live** on a real vocabulary, with a word from a student's corpus if one is to hand. The demo notebook (`week04_demo.ipynb`) has the whole mechanism in runnable code — the pair list, the random negatives, ninety seconds of visible pulling and pushing, then neighbours and the analogy trick failing honestly on a small corpus — so this can be shown live rather than described, and re-run afterwards by anyone who wants it. Close on pixels, five minutes: **a convolution written out in full** — nine numbers laid over nine pixels, multiply, add, one number out, slide one across — then a filter bank and a three-layer stack on a Met painting, where edges become corners and corners become regions. **CLIP** is where the two halves meet: a CNN reads the picture, a language model reads the caption, and the training rule is the one from the word vectors, which is why an untagged museum can be searched in plain English. Then one line each on static versus contextual vectors, and on the sentence embeddings Week 5 actually uses. |
| 1:00 | Break |
