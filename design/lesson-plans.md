...
**The critical-reading ritual — now the anchor of the discussion third.** "Look at This, then Question It" opens most weeks: admire a famous study, then name one interpretive choice its authors made and one way it could be wrong. From Week 5 onward, **flip it** — show the study and let the room name the choice before you do. That flip is how competency 8 becomes theirs rather than a performance you do for them. This was once hard-capped at five minutes; in the rebalanced course it grows into a real discussion — often the trial or debate format (claim, objection, rebuttal) — and in the project weeks it's joined by **peer critique** of students' own work in progress. Keep it one piece, well-chosen (depth over a gallery), but give it the time it now deserves.

**When minutes run out — and they will:** trim *proportionally* so the three-mode balance holds, rather than sacrificing one mode to rescue another. The old rule protected the lab and let discussion die first; the rebalanced course treats all three as load-bearing, so a tight session shaves a few minutes off each. The one floor that spans modes: every student should *build* something hands-on and *say* something in discussion — never a session that is all talk or all silent building.
...
| Time | Activity |
|---|---|
| 0:00 | **[Discussion] The embeddings debate (30 min) — the discussion third.** Caliskan's bias *warning* (embeddings launder human prejudice) vs. Soni/Klein's historical *recovery* (the same mirror used to surface the past): one fact about embeddings, opposite uses. Run it structured — claim, objection, rebuttal, then open floor — on the question the course turns on: when is reading a corpus's associations a *discovery*, and when is it *laundering bias*? The room names the choices first. This is the deepest lesson in the course (a method is good or bad for a purpose) and it seeds the Week 7–8 training-data thread. |
