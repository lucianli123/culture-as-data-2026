...
│   ├── week04_data_cookbook.ipynb         # fully worked (API pull + polite scrape)
...
**`kits/pivot-kit-corpora.md`** — 6–8 pre-vetted corpus-and-question pairs, each known to yield a finding in the time available, pop-weighted, spanning text and image, each tagged with which of the four tools it suits. Pull candidates from the starter corpora already named in `syllabus.md` (AITA judgments via HuggingFace Pushshift mirror, TMDb movie posters, Bluesky firehose slice, and the born-digital fiction sources — Reddit fiction like r/nosleep and r/HFY, Royal Road serials, public-domain genre fiction from Project Gutenberg — for the narrative projects). For each: corpus + access method + a question + the tool + why it's safe (data is reachable, licensed for analysis, and the question is answerable at this scale). Mark each `(PK)`.
...
- **`week04_data_cookbook.ipynb`** — a worked **API pull** (Met or Art Institute of Chicago, no key) and a small **polite scrape** (`requests` + `beautifulsoup4`) with an inline `robots.txt`/ToS/rate-limit/anti-republish checklist. This is also the corpus-existence-proof tool. `requests` + `beautifulsoup4` + `pandas`.
